...
public class gemas_al2 : MonoBehaviour
{
	...
	private void Start()
	{
		...
		if(fake == false)
		{
			if (manager.datosserial.gemaT[GemaID] == true)
			{
	...
	private void OnTriggerEnter(Collider col)
	{
		...
		if (col.gameObject.tag == "Player")
		{
			...
			{
				if (manager.datosserial.gemaT[GemaID] == false)
				{
					
				manager.datosserial.economia[0]++;
				manager.datosserial.gemaT[GemaID] = true;
				manager.guardar();
				jugador.musicajuego.Stop();
